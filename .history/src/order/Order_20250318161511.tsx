      ...
      <div className="  flex flex-wrap items-center justify-between gap-2 bg-stone-200 px-6 py-5">
        ...
        <p className=" text-xs text-stone-500">(Estimated delivery: {formatDate(estimatedDelivery)})</p>
            ...
            <ul className=" divide-stone-200 divide-y border-b border-t">
              {cart.map((item:typess)=>   <OrderItem key={item.pizzaId} isLoadingIngredients={fetcher.state == "loading"} ingredients={fetcher?.data?.find((res:tyeprrp)=> res.id === item.pizzaId)?.ingredients ?? []}  item={item} /> )}
            </ul>
...
export async function loader({params}:{params:Params<string>}) {
  ...
  const deta = await getOrder(orderid);
  return deta;
}
