      ...
      <Form method="POST">
        ...
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          <div className="grow">
            ...
            {formData?.phone && (
              <p className="text-sm mt-2 rounded-md bg-red-100 p-2 text-red-700">
                {formData.phone}
              </p>
            )}
          </div>
        ...
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center relative">
          ...
          {!position.latitude && !position.longitude && (
            <span className="absolute right-[3px] top-[34px] sm:top-[2px] md:top-[3px]">
              <Btn
                type="small"
                to=""
                disabled={loading}
        ...
        <div className="mb-12 flex items-center gap-5">
          <input
            ...
            name="priority"
            id="priority11"
            value={String(withPriority)} // تبدیل boolean به string
            onChange={(x) => setWithPriority(x.target.checked)}
          />
          <label className="font-medium" htmlFor="priority11">
        ...
        <div>
          ...
          />
          <Btn onClick={() => {}} type="primry" to="" disabled={issubmit || loading}>
            {issubmit ? "Placing order..." : <>{formatCurrency(totalprice)} Order now</>}
          </Btn>
        </div>
      </Form>
    </div>
